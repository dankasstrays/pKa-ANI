...
def tleap_vacuum(pdbfile, ssbond=None):
    ...
    if (ssbond is not None):
        for key, values in ssbond.items():
            ...
            bondtxt = "bond protein." + \
                str(values[0])+".SG protein."+str(values[1])+".SG \n"
...
def get_ssbond_rno(pdbin, ssbond):
    ...
    if os.path.exists("stdout_nonprot.pdb"):
        os.remove("stdout_nonprot.pdb")
    if os.path.exists("stdout_sslink"):
        os.remove("stdout_sslink")
    # if os.path.exists('pdb4amber.log'): os.remove('pdb4amber.log')
    if os.path.exists('tmp.pdb'):
        os.remove('tmp.pdb')
    if os.path.exists('pdb4amber.log'):
        os.remove('pdb4amber.log')
    if os.path.exists('stdout_renum.txt'):
        os.remove('stdout_renum.txt')
