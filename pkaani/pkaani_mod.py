...
def calculate_pka(pdbfiles, VHS_IDX, writefile=None):
    ...
    his_features = joblib.load(os.path.join(
        os.path.dirname(__file__), '/gstore/home/sastrys1/installations/pKa-ANI/pkaani/models/FHIS.joblib'))
    ...
    his_model = joblib.load(os.path.join(os.path.dirname(
        __file__), '/gstore/home/sastrys1/installations/pKa-ANI/pkaani/models/HIS_ani2x_FINAL_MODEL_F100.joblib'))
    ...
    for fpdb in pdbfiles:
        ...
        res, res_no, a_type, a_sym, a_no, pos, chainid, type_atm = pdb_arrays(
            atoms)
        ...
        for k, index in enumerate(pdball_resi):
            activation_i, aev_i, nk, ck, ok = get_indices(
                index, a_type, a_no, nk, ck, ok)
        ...
        for i, r in enumerate(ca_list):
            if(r in list(pkares) and i == VHS_IDX):
                ...
                a_symbols = []
                for i in res_aevi:
                    a_symbols.append(a_sym[i])

                ani_descriptors, features = get_desc_arrays(
                    ani, species_coordinates, aev, res_acti, res_aevi, a_symbols, a_type)
                ...
                if(writefile):
                    ...
                    writer.writerow(
                        [wres, mychain, '{:2.2f}'.format(estimate_pka[0])])

                # resdict={}
                # resdict[lrnum]=lres,estimate_pka[0]
                # pkadict[fpdb].append(resdict)
                pkadict[fpdb] = estimate_pka[0]
